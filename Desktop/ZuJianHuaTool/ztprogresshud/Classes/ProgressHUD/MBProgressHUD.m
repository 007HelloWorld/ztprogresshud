...
static const CGFloat MBHideDelayDuration = 2.5f;
...
+ (instancetype)showMessageInView:(UIView *)view withText:(NSString *)hudText {
    ...
    if ([ZTUtil isBlankString:hudText]) {
    ...
    hud.label.text = hudText;
    [hud hideAnimated:YES afterDelay:MBHideDelayDuration];
    return hud;
}
...
+ (instancetype)showBlockedHUDInView:(UIView *)view withText:(NSString *)hudText {
    ...
    MBProgressHUD *hud = [MBProgressHUD showHUDInView:view withText:hudText];
    hud.square = YES;
    hud.userInteractionEnabled = YES;
    return hud;
}
...
//    MBProgressHUD *hud = [self HUDForView:view];
    for (UIView *subview in view.subviews) {
        if ([subview isKindOfClass:self]) {
            [(MBProgressHUD *)subview setRemoveFromSuperViewOnHide:YES];
            [(MBProgressHUD *)subview hideAnimated:animated];
        }
    }
//    if (hud != nil) {
//        hud.removeFromSuperViewOnHide = YES;
...
#pragma mark - Lifecycle  (_contentColor内容颜色，包含UIActivityIndicatorView颜色和label文字颜色)
...
#pragma mark - Lifecycle  (_color可以是bezelView或者backgroundView的背景色)

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = MBProgressHUDBackgroundStyleSolidColor;
        //bezelView或者backgroundView的背景色
